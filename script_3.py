...
for card in eds_cards + mod:
    ...
    output.write(card + '\t' + card_info[card]['type'] + '\t' + attribute + '\t' + card_info[card]['race'] + '\t' + str(level) + '\t' + str(atk) + '\t' + str(defn) + '\n')
        ...
        # output.write(desc.replace('"', '\\"').replace('\r\n', ' ').replace('\n', ' ')[:478].ljust(478, '�') + '\n')
        # output.write('+#' + card + '#, (' + str(int(defn/10)) + ' ' + str(int(atk/10)) + ' ' + card_info[card]['type'] + ' ' + card_info[card]['race'] + ' ' + str(level) + ' ' + attribute + ')\n')
        # output.write('#define CARD_NUMBER_' + card.upper().replace('"', '_').replace("'", '_').replace('#', '_').replace(' ', '_').replace('-', '_').replace('.', '_').replace(',', '_').replace('__', '_').replace('__', '_') + ' ' + str(counter) + '\n')
#         output.write(
#     '    [CARD_NUMBER_' + card.upper().replace('"', '_').replace("'", '_').replace('#', '_').replace(' ', '_').replace('-', '_').replace('.', '_').replace(',', '_').replace('__', '_').replace('__', '_') + ''']
#     {
